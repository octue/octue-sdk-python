...
class Twine:
    ...
    def _get_schema(self, strand):
        ...
        if strand == "twine":
            ...
            return jsonlib.loads(
                importlib_resources.files("octue.twined.schema")
                .joinpath("twine_schema.json")
    ...
    def _validate_twine_version(self, twine_file_twined_version):
        ...
        installed_twined_version = importlib.metadata.version("octue")
        logger.debug(
    ...
    def _validate_all_expected_datasets_are_present_in_manifest(self, manifest_kind, manifest):
        ...
        :raise octue.twined.exceptions.InvalidManifestContents: if one or more of the expected non-optional datasets is missing
        :return None:
        """
    ...
    def validate_credentials(self, *args, dotenv_path=None, **kwargs):
        r"""Validate that all credentials required by the twine are present.
        ...
        dotenv_path = dotenv_path or os.path.join("twined", ".env")
