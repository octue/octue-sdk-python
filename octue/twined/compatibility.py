...
def is_compatible(parent_sdk_version, child_sdk_version):
    """Check if two versions of Twined are compatible according to empirical testing. If there's no information on one
    of the versions, a warning is issued and they're treated as compatible.

    :param str parent_sdk_version: the semantic version of Twined running the parent
    :param str child_sdk_version: the semantic version of Twined running the child
    :return bool:
    """
    if (
...
def warn_if_incompatible(sender_sdk_version, recipient_sdk_version):
    ...
    :param str|None sender_sdk_version: the version of the Twined running on the sender
    :param str|None recipient_sdk_version: the version of the Twined running on the recipient
    :return None:
    """
    ...
    if missing_service_version_information:
        logger.warning(
            "The %s couldn't be checked for compatibility with this service because its Twined version wasn't "
    ...
    if not is_compatible(sender_sdk_version, recipient_sdk_version):
        logger.warning(
            "The sender's Twined version %s is incompatible with the recipient's version %s. Please update either "
