...
def load_test_fixture_from_diagnostics(path):
    """Load a test fixture from service diagnostics downloaded using the `octue twined question diagnostics` CLI
...
def _load_child_emulators(path):
    ...
    return tuple(ChildEmulator(events=question["events"]) for question in questions)
