...
@click.group()
@click.option('--id', default=None, show_default=True, help='Id of the analysis being undertaken. None (for local use) prevents registration of the analysis or any of the results.')
# TODO Add --log-level option
@click.option('--skip-checks/--no-skip-checks', default=False, is_flag=True, show_default=True, help='Skips the input checking. This can be a timesaver if you already checked the data directory once, and its a big manifest.')
@click.option('--force-reset/--no-force-reset', default=True, is_flag=True, show_default=True, help='Forces a reset of analysis cache and outputs [For future use, currently not implemented)')
@click.option('--data-dir', type=click.Path(), default='.', show_default=True, help='Absolute or relative path to the data directory (which will contain config, manifest files and <data_dir>/input <data_dir>/output folders).')
@click.option('--input-dir', type=click.Path(), default='<data-dir>/input', show_default=True, help='Absolute or relative path to a folder containing input files.')
