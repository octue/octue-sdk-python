...
.. code-block:: python
    ...
    datafile.cluster = 0
    datafile.sequence = 3
    datafile.labels = {"manufacturer:Vestas", "output:1MW"}
...
.. code-block:: python
    ...
    sequence = 2
    labels = {"cleaned:True", "type:linear"}


    with Datafile(path="path/to/local/file.dat", sequence=sequence, labels=labels, mode="w") as datafile, f:
...
.. code-block:: python
    ...
    datafile = Datafile(path="path/to/local/file.dat", sequence=sequence, labels=labels)
    datafile.to_cloud(project_name="my-project", bucket_name="my-bucket", path_in_bucket="path/to/data.dat")
