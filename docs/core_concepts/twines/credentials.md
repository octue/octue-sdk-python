...
- query a database, or
- connect to a socket (for receiving or emitting values or emitting, monitors or logs),
...
(whose `NAMES_SHOULD_BE_SCREAMING_SNAKE_CASE`) with a reminder of the
purpose.
...
variables. Credentials may also reside in a `.env` file in the current directory,
